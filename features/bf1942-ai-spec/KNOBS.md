...
| strategic | beach order | a `LandingCraft` unit ordered to an area that uses a zone for it: `WPBeachLanding` on the attached zone nearest it; any other target: the first zone user on the shortest area path, `WPMoveToBeachLanding`; else the ordinary `WPMoveTo` | `orderNormalBot` 0x08640bd0 (no route 0x08640d05, one area 0x08641428, route loop 0x086416dd), `getClosestLandingZone` 0x086401e0 (AI-96) | `strategic-ai.js _orderBeach`, `doctrine-landing.js beachTarget`, `areaPath`, `craftArea` | ENGINE; the area path INVENTION |
...
| strategic | beach order radius | 10 for `WPBeachLanding`; 5 for `WPMoveToBeachLanding` (the engine's `max(5, 0.25 side radius + 2 bounding)` of the last route area, 1.0 with none) | `getWayPointRadius` 0x08536500; ctor 0x08537ce0, 0x086416be (AI-96, AI-98) | `LANDING.waypointRadius`, `moveToRadiusMin`, `moveToRadiusNoRoute` | ENGINE; the no-route value for a routed order INVENTION |
...
| strategic | beach leg | the boat's helm straight at the beach point, never done; the ramp (`PIPitch`) held | `BBPGotoWaypointBoat::createPlan` 0x085b8c50 (AI-98) | `bot-plans.js BoatMoveToDirect` | ENGINE; the ramp not built |
| change | landing craft crew | out when the craft is in any zone, under 2 m/s and the infantry map holds it, or tipped; urgency 4.0; no voluntary bail, no other hull | `BBChangeLandingCraft::calculateUrgency` 0x085602b0 (2.0 at 0x8560c60, 4.0 at 0x085608ea; AI-99) | `doctrine-landing.js craftBailReason`, `LANDING.bailSpeed`; `bot-mount.js LANDING_BAIL_URGENCY`, `LANDING_CRAFT_RE` | ENGINE; `isTouchingLand` (the infantry map) and the tip test (`upright` 0.6914) INVENTION stand-ins |
