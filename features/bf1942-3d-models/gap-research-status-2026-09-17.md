...
| 8 | Soldiers in map flythrough | **PARTIAL** | `placeSpawnSoldiers` in `map.html` loads assault-kit pose (or bare soldier glb) per team and clones to every non-paratrooper `soldierSpawns` pad. Still decoration-only (no AI / idle anim). |
...
| 12 | `envmap` materials | **OPEN** | `.rs` bool parser has `twosided|transparent|lighting|textureFade` only — no `envmap` (`bf42/rs.py:32-33`). Level cubemap export is for sky/water reflection (`extract_map.py:1804-1808`), not per-material glass. |
| 13 | Non-Conquest modes | **OPEN** | `GAMEPLAY_MODES` + `find_gameplay_mode` pick **one** directory, Conquest first (`bf42/level.py:703-720`). `load_gameplay_objects` reads only that mode (`750-773`). Other modes' vehicle/soldier placements stay unread when Conquest exists. |
