...
**2. The eye is 1.56 m up, and the stance deltas are declared exactly.**
`setPoseCameraPos` gives 0.65 / 0.12 / −0.70 for standing / crouching / lying,
so crouch is 0.53 m below standing and prone is 1.35 m below it. The absolute
comes from `center1pHands -0.12/-1.56/0.1`, the offset that places the 1P rig
relative to the camera, and is confirmed to 1.7 cm by an independent route
(§2.1). Cross-checked against the posed head bone: measured stance deltas
−0.564 m and −1.322 m against the declared −0.53 and −1.35.

**3. Movement speed is not declared anywhere — but the footstep clock is, and
it plus the stride gives you the answer.** `SoldierSound.setRunFrequency 0.36`
/ `setWalkFrequency 0.66` / `setCrouchFrequency 0.50` / `setCrawlFrequency 0.6`
are seconds between footsteps; the locomotion clips are strictly in-place
(measured net root displacement ≤ 2 cm on every one), and the step length is
measurable off the posed feet. Run 2.28 m/s, walk 1.08 m/s, crouch 1.58 m/s,
crawl 0.69 m/s. **The audit's cited `maxSpeed 5.0` is not the run speed** — it
is an AI planning figure, and the Hanomag half-track declares the same 5.0.
...
| stance | declared offset | eye above feet | posed head bone | posed helmet bone `A` |
|---|---|---|---|---|
| standing | `0/0.65/0` | **1.56** | 1.638 | 1.754 |
| crouching | `0/0.12/0` | **1.03** | 1.074 | 1.194 |
| lying | `0/-0.7/0` | **0.21** | 0.316 | 0.420 |
...
**So where is the speed?** Nowhere, directly. Three lines of evidence:
...
| gait | clip | frames | step length | footstep period | **derived speed** |
|---|---|---|---|---|---|
| run | `3PRunLower` | 13 | 0.821 m | 0.36 s | **2.28 m/s** |
| walk (Shift) | `3PWalkLower` | 24 | 0.711 m | 0.66 s | **1.08 m/s** |
| crouch move | `3PCrouchForwardLower` | 18 | 0.790 m | 0.50 s | **1.58 m/s** |
| prone crawl | `3PLieForwardLower` | 18 | 0.414 m | 0.60 s | **0.69 m/s** |

Ratios 1 : 0.47 : 0.69 : 0.30. These are derived, not declared — label them as
such in code and make them one constant block a future measurement can replace.
...
is engine-side and **UNVERIFIED**. Nothing in `Objects.rfa` or `Game.rfa`
declares a gravity constant for a soldier; `gravityModifier` exists only on
projectiles. `gunfire.js:67` already defines `GRAVITY = 9.81`; use that and tune
the impulse
by eye.
...
So: a **0.31 m radius vertical capsule, 1.75 m tall standing and 1.20 m
...
segment against water / terrain / hulls at 1.2–2.1 µs, and
...
their in-game speed). The real figure has to be derived from the footstep clock
and the stride (§2.3), which lands at 2.28 m/s.
...
- **Jump impulse, step-up height, slope limit, movement capsule radius.** None
  exist in vanilla data. Ours to choose; label them.
