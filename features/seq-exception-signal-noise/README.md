...
detection failure ~60 min later. Not a 02:00 retry: after a failed 02:00 run
the catch waits 5 min then the loop sleeps until tomorrow 02:00, and
communities are still frozen since 2026-08-20.

Separately, `GET /stats/players` (default page) and search `query=Frankie`
returned 400 `An item with the same key has already been added. Key: Frankie`.
Liveservers showed Frankie once on `Badewiese [Public DC Server]` (desertcombat,
el alamein). Player-detail had that live session (`lastSeenTime` 03:03:26,
started 02:59:56) plus a stale IsActive row on `MoonGamers.com | Est. 2004`
(`lastSeenTime` 02:58:26, battleaxe). The controller catches `ArgumentException`
and returns 400 without logging, so this is **not** the Seq page. Search of
nico / Cosmik_Debris / Paciencia / JUANI / Angela Merkel / Americanator /
Mikael Skillt / Ho-Chi Minh all 200.

The 02:02 branch (`cursor/site-error-analysis-84dc`) never opened a PR, so
