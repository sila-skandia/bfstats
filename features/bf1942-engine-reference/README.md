...
| **Radio commands and the message log** (2026-09-23). The F1..F8 menu, its messages, voices and chat lines; the three-section kill / game-information / chat log | radio key handler 0x006D42A0, team and shouted receive 0x006D3400 / 0x006D2790, grid square 0x006ACDB0, `addChatMessageInternal` 0x006A89C0 and the log update 0x006A8160, `GameServer::radioMessage` (lnxded 0x0813A120) | [`../radio-and-chat-log/README.md`](../radio-and-chat-log/README.md). Open: `BfMenu+0x6DC` (conquest's radio game mode), bots reacting to radio. The kill line's weapon word was settled 2026-09-24 (the stamped template through `Locale::getWide`, the key itself on a miss) |
| **The pilot's mouse-look key** (2026-09-25). A pilot's mouse looks only while `c_PIMouseLook` (Left Shift) is held, the released view eases back, and holding the key zeroes rudder and stick | ledger MLK-1…MLK-6: the Camera template's `toggleMouseLook` byte (MLK-1), `BFPlayer::handleInput`'s router (MLK-2), the 0.75-a-tick recentre (MLK-3), every view alike (MLK-4), exactly the aircraft pilots' cameras in vanilla, XPack1 and XPack2 (MLK-5) | [`../pilot-mouse-look/README.md`](../pilot-mouse-look/README.md). Open: the word is not extracted, so mods get the shipped rule (pilot of an `air` hull); the look law while held is still the viewer's |
