...
| **Bots and the AI subsystem** (2026-09-21). Research only — nothing is built | ledger AI-1…AI-25. A bot is an ordinary player whose `PlayerInput` the engine writes for it (AI-1); the AI is a time budget, not a per-bot loop (AI-6); difficulty is one knob with ten call sites that vanilla never sets (AI-14, AI-15) | [`../bf1942-ai-research-2026-09-21/README.md`](../bf1942-ai-research-2026-09-21/README.md): the whole model, the level data, and a staged cost estimate. Open: the `Pathfinding/*.raw` row encoding (AI-13), the sensing model, the urgency generators. **AI-21 is a verified negative**: the retail Instant Battle AI SKILL slider is inert, so wiring one is an improvement on the game and must be labelled one |
