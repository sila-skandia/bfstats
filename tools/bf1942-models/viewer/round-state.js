 ...
 *
 *  - the round starts both sides at `Game.setNumberOfTickets`, which
 *    `scene.json.tickets` already carries;
 ...
 *    command);
 *  - and a team bleeds one ticket per `60 / rate` seconds, where `rate` is its
 *    own `Game.setTicketLostPerMin` (`scene.json.tickets.lossPerMin`), while
 *    the ENEMY's summed `areaValue` over the points it holds is greater than
 *    99.
 *
 ...
 *                the table above;
 *  - `tickets`   `{ team1, team2 }`, the round's starting counts;
...
export function createRoundState({
  ...
  const round = {
    ...
    tickets: {
      1: Math.max(0, Math.trunc(Number(tickets?.team1 ?? 0))),
      2: Math.max(0, Math.trunc(Number(tickets?.team2 ?? 0))),
    },
  ...
  for (const team of [1, 2]) {
    ...
    // The engine builds the countdown as 60 / (16 * 0.0625 * rate) and
    // GameServer::init writes the 16, so the interval is 60 / rate seconds.
    // A rate of zero means the level declared no bleed for that side, and an
    // infinite interval is exactly "never".
    intervals[team] = rate > 0 ? 60 / rate : Infinity;
