...
This document is the **single source of truth** for unmocked mobile
pages. Every pattern here is backed by a shared class in
`modern-minimal.css` — use the class names rather than re-rolling local
CSS. If a layout doesn't fit one of these patterns, ask for a mock
before improvising.

Companion docs: [`PATTERNS.md`](./PATTERNS.md) (recipes + tokens),
...
See [`PATTERNS.md` → Recipe: Always paginate list endpoints](./PATTERNS.md).
