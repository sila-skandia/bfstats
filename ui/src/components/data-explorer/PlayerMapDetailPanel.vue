...
<template>
  <div class="flex flex-col h-full bg-[#05050a] p-4 sm:p-8 overflow-y-auto custom-scrollbar">
    ...
    >
      ...
      <div class="font-mono text-[9px] text-cyan-400 uppercase tracking-[0.3em]">
        Decoding_Map_Telemetry...
      </div>
    </div>
    ...
    >
      ...
      <p class="text-red-400 font-mono text-xs uppercase tracking-widest mb-6">
        {{ error }}
      </p>
      <button
        class="px-6 py-2 bg-white/5 border border-white/10 text-white text-[10px] font-mono font-black uppercase tracking-widest hover:bg-white/10 transition-all"
        @click="loadData"
      >
        Retry_Link
      </button>
    ...
    >
      ...
      <section class="space-y-4">
        <div class="flex items-center gap-4">
          ...
          <h3 class="text-[10px] font-mono font-black text-white uppercase tracking-[0.3em]">
            Operative_Summary
          </h3>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div
            ...
            :key="stat.label"
            class="bg-black/40 border border-white/5 p-5 rounded-xl hover:bg-white/[0.02] transition-all"
          >
            <div class="text-[8px] font-mono text-slate-500 uppercase tracking-widest mb-1">
              {{ stat.label }}
            </div>
            <div
      ...
      <section class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <select
              ...
              class="bg-black/40 border border-white/10 rounded px-3 py-1 text-[9px] font-mono text-cyan-400 focus:outline-none focus:border-cyan-500/50"
              @change="loadRankings"
            >
              <option value="">
                ALL_NODES
        ...
        <div class="relative min-h-[300px] border border-white/5 rounded-xl overflow-hidden bg-black/20">
          <div
            v-if="isRankingsLoading"
            class="absolute inset-0 z-10 flex items-center justify-center bg-[#05050a]/40 backdrop-blur-sm"
          >
          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-white/5 font-mono">
              <tr
                ...
                :class="{ 'bg-cyan-500/10': p.playerName === playerName }"
              >
                <td class="p-3">
                  <div
                    class="w-6 h-6 mx-auto flex items-center justify-center rounded border text-[9px] font-black shadow-[inset_0_0_5px_rgba(0,0,0,0.5)]"
                    :class="[getRankClass(p.rank), 'border-current/20']"
                  >
                    {{ p.rank }}
                  </div>
                </td>
        ...
        <div
          v-if="totalPages > 1"
          class="flex items-center justify-center gap-4 pt-4 border-t border-white/5"
        >
          <button
            :disabled="currentPage === 1"
            class="text-slate-500 hover:text-white disabled:opacity-20"
