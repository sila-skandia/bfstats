...
<template>
  <!-- Loading skeleton -->
  <div
    v-if="isLoading && !leaderboardsData"
    class="space-y-4 py-2 animate-pulse"
  >
    ...
    <div class="grid grid-cols-2 gap-3">
      <div
        v-for="i in 4"
        :key="i"
        class="h-40 bg-neutral-800/50 rounded-lg"
      />
    </div>
  </div>
  ...
  >
    ...
    <div class="flex items-center gap-3 flex-wrap">
      <div class="flex items-center rounded-md bg-black/40 p-0.5 border border-neutral-700/50">
        ...
        >
          <span
            v-if="isLoading && selectedTimePeriod === period[0]"
            class="inline-block w-3 h-3 border border-neutral-500 border-t-neutral-200 rounded-full animate-spin align-middle"
          />
      ...
      >
        ...
        <div class="relative rounded-lg overflow-hidden bg-gradient-to-b from-yellow-900/10 to-neutral-900/50 border border-yellow-600/20 shadow-[0_0_15px_-3px_rgba(234,179,8,0.1)]">
          ...
          <div class="relative text-center py-6 px-4">
            <div class="mb-2 text-[10px] font-bold uppercase tracking-widest text-yellow-500/80">
              Current Champion
            </div>
            <router-link
            ...
            >
              <span class="text-xl sm:text-3xl font-black tracking-tight text-white drop-shadow-[0_0_10px_rgba(234,179,8,0.3)] group-hover/champ:text-yellow-100 transition-colors duration-300">
                <PlayerName
                  :name="currentTopPlacements[0].playerName"
        ...
        <div
          v-if="currentTopPlacements.length > 1"
        ...
        >
          ...
          >
            ...
            <div class="relative z-10">
              <div class="text-[9px] font-bold uppercase tracking-widest text-slate-500 mb-1">
                Runner Up
              </div>
              <router-link
              ...
              >
                <PlayerName
                  :name="currentTopPlacements[1].playerName"
                  source="server-leaderboards"
                />
              </router-link>
          ...
          >
            ...
            <div class="relative z-10">
              <div class="text-[9px] font-bold uppercase tracking-widest text-amber-700 mb-1">
                Third Place
              </div>
              <router-link
              ...
              >
                <PlayerName
                  :name="currentTopPlacements[2].playerName"
                  source="server-leaderboards"
                />
              </router-link>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <div class="bg-neutral-900/40 rounded-lg p-4 border border-cyan-900/30 relative overflow-hidden group/card hover:border-cyan-700/30 transition-colors">
        <div class="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-cyan-500/50 to-transparent" />
        <div class="text-[10px] font-bold uppercase tracking-[0.15em] text-cyan-500/80 mb-3 flex items-center gap-2">
          <span class="w-1.5 h-1.5 rounded-full bg-cyan-500" /> Most Active
        </div>
        ...
        <div
          v-else
          class="text-xs text-neutral-500 py-3 text-center"
        >
          No data
        </div>
      </div>
      ...
      <div class="bg-neutral-900/40 rounded-lg p-4 border border-emerald-900/30 relative overflow-hidden group/card hover:border-emerald-700/30 transition-colors">
        <div class="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-emerald-500/50 to-transparent" />
        <div class="text-[10px] font-bold uppercase tracking-[0.15em] text-emerald-500/80 mb-3 flex items-center gap-2">
          <span class="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Top Scores
        </div>
      ...
      <div class="bg-neutral-900/40 rounded-lg p-4 border border-purple-900/30 relative overflow-hidden group/card hover:border-purple-700/30 transition-colors">
        <div class="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-purple-500/50 to-transparent" />
        <div class="flex items-center justify-between gap-2 mb-3 relative z-10">
          <div class="text-[10px] font-bold uppercase tracking-[0.15em] text-purple-500/80 flex items-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-purple-500" /> Elite K/D
          </div>
          <select
          ...
          >
            <option
              v-for="n in [5, 10, 15, 20, 25, 30, 50, 100]"
              :key="n"
              :value="n"
            >
              {{ n }}+ rnds
      ...
      <div class="bg-neutral-900/40 rounded-lg p-4 border border-rose-900/30 relative overflow-hidden group/card hover:border-rose-700/30 transition-colors">
        <div class="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-rose-500/50 to-transparent" />
        <div class="flex items-center justify-between gap-2 mb-3 relative z-10">
          <div class="text-[10px] font-bold uppercase tracking-[0.15em] text-rose-500/80 flex items-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-rose-500" /> Kill Rate
          </div>
          <select
            :value="localMinRoundsForKillBoards"
