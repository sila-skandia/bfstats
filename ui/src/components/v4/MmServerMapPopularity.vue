...
import MmServerMapDrawer from '@/components/v4/MmServerMapDrawer.vue'
...
const localServerName = ref(props.serverName)
...
const openDrillIn = (mapName: string) => {
...
const totalFilteredRounds = computed(() =>
  filteredSummaries.value.reduce((acc, m) => acc + m.totalRounds, 0)
)

const maxRounds = computed(() =>
  Math.max(1, ...filteredSummaries.value.map(m => m.totalRounds))
)

const maxAvgPlayers = computed(() =>
  Math.max(1, ...filteredSummaries.value.map(m => m.avgConcurrentPlayers))
)
...
const avgPlayersChartData = computed(() => {
  const sorted = [...filteredSummaries.value].sort((a, b) => b.avgConcurrentPlayers - a.avgConcurrentPlayers)
  return {
...
const getRoundSharePct = (rounds: number): number => {
  if (totalFilteredRounds.value === 0) return 0
  return (rounds / totalFilteredRounds.value) * 100
}

const getPopularityTierClass = (sharePct: number, rankIdx: number): string => {
  if (rankIdx === 0 || sharePct >= 25) return 'mm-mp-bar--elite'
  if (sharePct >= 15) return 'mm-mp-bar--good'
  if (sharePct >= 8) return 'mm-mp-bar--mid'
  return 'mm-mp-bar--low'
}
...
const loadData = async () => {
  ...
  try {
    ...
    if (!result) {
      error.value = 'Server map data not found'
      return
    }
    data.value = result
    ...
    error.value = e?.message || 'Failed to load map popularity data'
  } finally {
    loading.value = false
  }
}

const fetchServerNameIfNeeded = async () => {
  if (localServerName.value || !props.serverGuid) return
  try {
    const { fetchServerDetail } = await import('@/services/dataExplorerService')
    const detail = await fetchServerDetail(props.serverGuid)
    localServerName.value = detail.name
  } catch { /* non-critical */ }
}

const backToServer = () => {
  if (localServerName.value) {
    router.push(`/v4/servers/detail/${encodeURIComponent(localServerName.value)}`)
  } else {
    router.back()
  }
}
...
<template>
  <div class="mm-mp" :class="{ 'mm-mp--standalone': standalone, 'mm-container': standalone, 'mm-section': standalone }">
    <!-- Standalone header / back navigation -->
    <template v-if="standalone">
      <div class="mm-meta-row" style="margin-bottom: 14px">
        ...
        >← {{ localServerName ? `Back to ${$pn(localServerName)}` : 'Back' }}</a>
      </div>

      <h1 class="mm-display">
        Map popularity
        <span v-if="localServerName" class="mm-display__muted">· {{ $pn(localServerName) }}</span>
      </h1>
    ...
    <div v-if="showMapFilter && allMaps.length > 0" class="mm-mp__filter-panel">
      <div class="mm-mp__filter-head">
        <span class="mm-eyebrow">Filter active maps</span>
    ...
    <hr class="mm-rule" style="margin-top: 20px" />
    ...
    <template v-else>
      ...
      <section style="margin-top: 20px">
        <div class="mm-eyebrow mm-eyebrow--strong">Average players per map</div>
      ...
      <!-- Time-of-day heatmap -->
      <section style="margin-top: 20px">
        <div class="mm-eyebrow mm-eyebrow--strong">Popularity by time of day</div>
        <p class="mm-card__hint" style="margin-top: 4px">
          Average player count by your local hour. Brighter = more popular.
        </p>
        <div class="mm-mp__heatmap" style="margin-top: 14px">
          ...
          >
            <div class="mm-mp__heat-label" :title="summary.mapName" @click="openDrillIn(summary.mapName)">
              <span class="mm-mp__chip-dot" :style="{ backgroundColor: getMapColor(summary.mapName) }" />
              <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: pointer">{{ summary.mapName }}</span>
            </div>
      ...
      <!-- Summary table with relative colored bars and drill-in click -->
      <section style="margin-top: 20px">
        <div class="mm-eyebrow mm-eyebrow--strong" style="margin-bottom: 12px">Map rotation & popularity summary</div>
        <table class="mm-list mm-list--dense mm-mp-table">
          <thead>
            <tr>
              <th style="width: 36px">#</th>
              <th>Map</th>
              <th class="is-num" style="width: 120px">Rounds</th>
              <th class="is-num" style="width: 150px">Avg players</th>
              <th class="is-num" style="min-width: 180px">Popularity share</th>
          ...
          <tbody>
            <tr
              ...
              :key="s.mapName"
              class="mm-mp-table__row"
              @click="openDrillIn(s.mapName)"
            >
              ...
              <!-- Rounds count with subtle relative bar -->
              <td class="is-num" data-cell-label="Rounds">
                <div class="mm-mp-cell">
                  <span class="mm-mp-cell__num">{{ s.totalRounds }}</span>
                  <div class="mm-mp-cell__bar mm-list__col--hide-sm">
                    <div
                      class="mm-mp-cell__fill mm-mp-cell__fill--neutral"
                      :style="{ width: Math.round((s.totalRounds / maxRounds) * 100) + '%' }"
                    />
                  </div>
                </div>
              </td>
              <!-- Avg players with relative bar -->
              <td class="is-num" data-cell-label="Avg players">
                <div class="mm-mp-cell">
                  <span class="mm-mp-cell__num">{{ s.avgConcurrentPlayers.toFixed(1) }}</span>
                  <div class="mm-mp-cell__bar mm-list__col--hide-sm">
                    <div
                      class="mm-mp-cell__fill mm-mp-cell__fill--accent"
                      :style="{ width: Math.round((s.avgConcurrentPlayers / maxAvgPlayers) * 100) + '%' }"
                    />
                  </div>
                </div>
              </td>
              <!-- Popularity Share with vibrant tiered relative bar -->
              <td class="is-num" data-cell-label="Share">
                <div class="mm-mp-share">
                  <div class="mm-mp-share__track">
                    <div
                      class="mm-mp-share__fill"
                      :class="getPopularityTierClass(getRoundSharePct(s.totalRounds), idx)"
                      :style="{ width: Math.max(4, Math.round(getRoundSharePct(s.totalRounds))) + '%' }"
                    />
                  </div>
                  <span class="mm-mp-share__val">{{ getRoundSharePct(s.totalRounds).toFixed(1) }}%</span>
                </div>
              </td>
              <td data-cell-label="" class="mm-list__col--hide-sm">
                <span class="mm-eyebrow mm-mp-table__drill">Drill →</span>
              </td>
            </tr>
          </tbody>
        </table>
...
.mm-mp__heat-label:hover {
  color: var(--mm-accent);
}

@media (max-width: 640px) {
  .mm-mp__heat-label { width: 5.5rem; }
}
...
/* Map Summary Table Styling */
.mm-mp-table__row {
  cursor: pointer;
  transition: background-color 0.15s ease;
}

.mm-mp-table__row:hover {
  background: var(--mm-bg-soft);
}
...
.mm-mp-table__row:hover .mm-mp-table__drill {
  color: var(--mm-accent);
  transform: translateX(2px);
}

/* Numeric Cell with inline magnitude bar */
.mm-mp-cell {
  display: inline-flex;
  align-items: center;
  gap: 10px;
...
.mm-mp-cell__fill--neutral { background: var(--mm-ink-faint); }
.mm-mp-cell__fill--accent { background: var(--mm-accent); }

/* Popularity Share Relative Progress Bar */
.mm-mp-share {
  display: flex;
  align-items: center;
  gap: 12px;
...
/* Theme color tiers for relative popularity bars */
.mm-mp-bar--elite {
  background: linear-gradient(90deg, #7d8849, #b4c060);
  box-shadow: 0 0 8px rgba(180, 192, 96, 0.4);
}

.mm-mp-bar--good {
  background: var(--mm-success, #7da34c);
}

.mm-mp-bar--mid {
  background: var(--mm-load-busy, #c5a23a);
}

.mm-mp-bar--low {
  background: var(--mm-ink-muted, #8a8a8a);
}
