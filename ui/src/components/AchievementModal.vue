      ...
      >
        ...
        >
          ...
          <div class="flex items-center justify-between p-6 border-b border-white/5 bg-white/[0.02]">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 class="text-2xl font-black text-white uppercase italic tracking-tighter">{{ achievement.achievementName }}</h3>
              </div>
            </div>
            <button
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-8 space-y-8">
            ...
            <div v-if="isRoundPlacementAchievement && parsedMetadata" class="space-y-8">
               <div class="flex flex-col md:flex-row items-center gap-8 bg-black/40 border border-white/5 rounded-2xl p-8 relative overflow-hidden group">
                  ...
                  <div class="relative z-10 flex-1 text-center md:text-left">
                    <div class="mb-4">
                      <div class="text-[10px] font-mono text-cyan-400 uppercase tracking-[0.4em] mb-1">Final_Position</div>
                      <div class="text-6xl font-black italic tracking-tighter" :class="getPlacementColor(achievement.value || 1)">
                    ...
                    <div class="grid grid-cols-3 gap-4 border-t border-white/10 pt-6">
                       <div>
                         <div class="text-[8px] font-mono text-slate-500 uppercase mb-1">Kills</div>
                         <div class="text-xl font-black text-white font-mono">{{ parsedMetadata.Kills }}</div>
                       </div>
                       <div>
                         <div class="text-[8px] font-mono text-slate-500 uppercase mb-1">Score</div>
                         <div class="text-xl font-black text-neon-gold font-mono">{{ parsedMetadata.Score }}</div>
                       </div>
                       <div>
                         <div class="text-[8px] font-mono text-slate-500 uppercase mb-1">E_Rate</div>
                         <div class="text-xl font-black text-emerald-500 font-mono">{{ (parsedMetadata.Kills / Math.max(1, parsedMetadata.Deaths)).toFixed(1) }}</div>
                       </div>
                    </div>
                  </div>
               </div>
            ...
            <div v-else-if="isTeamVictoryAchievement && parsedMetadata" class="space-y-8">
               <div class="flex flex-col md:flex-row items-center gap-8 bg-black/40 border border-white/5 rounded-2xl p-8 relative overflow-hidden group">
                  ...
                  <div class="relative z-10 flex-1 text-center md:text-left">
                    <div class="text-[10px] font-mono text-emerald-400 uppercase tracking-[0.4em] mb-2">Team_Victory_Confirmed</div>
                    <div class="flex items-center justify-center md:justify-start gap-4 mb-6">
                       <div class="text-center">
                         <div class="text-2xl font-black text-emerald-400 uppercase tracking-tighter italic leading-none">{{ parsedMetadata.WinningTeamLabel }}</div>
                         <div class="text-lg font-mono text-white mt-1">{{ parsedMetadata.WinningTeamTickets }}</div>
                       </div>
                       <div class="text-slate-700 font-black italic">VS</div>
                       <div class="text-center opacity-40">
                         <div class="text-lg font-black text-red-400 uppercase tracking-tighter italic leading-none">{{ parsedMetadata.LosingTeamLabel }}</div>
                         <div class="text-sm font-mono text-white mt-1">{{ parsedMetadata.LosingTeamTickets }}</div>
                       </div>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
               <!-- Explanation -->
               <div v-if="badgeDescription" class="col-span-1 md:col-span-2 bg-white/5 border border-white/10 rounded-xl p-5">
               ...
               <!-- Metadata: Map/Server -->
               <div v-if="achievement.mapName" class="bg-black/20 border border-white/5 rounded-xl p-4">
                  <div class="text-[8px] font-mono text-slate-600 uppercase mb-2">Location_Context</div>
                  <div class="flex flex-col">
                    <span class="text-xs font-bold text-white uppercase tracking-tight">{{ achievement.mapName }}</span>
                    <span v-if="parsedMetadata?.ServerName" class="text-[9px] font-mono text-slate-500 uppercase truncate mt-0.5">{{ parsedMetadata.ServerName }}</span>
                  </div>
               </div>

               <!-- Metadata: Tier -->
               <div v-if="achievement.tier" class="bg-black/20 border border-white/5 rounded-xl p-4">
                  <div class="text-[8px] font-mono text-slate-600 uppercase mb-2">Technical_Tier</div>
            ...
            <!-- Operational Links -->
            <div v-if="achievement.roundId && playerName" class="flex justify-center">
              <router-link 
          ...
          <div class="p-4 border-t border-white/5 bg-black/40 flex justify-between items-center opacity-30">
             <div class="text-[8px] font-mono text-slate-500 uppercase tracking-widest">UID: 0x{{ achievement.achievementId.substring(0,8) }}</div>
             <div class="text-[8px] font-mono text-slate-500 uppercase tracking-widest">Terminal_Status: SECURE</div>
          </div>
        </div>
      </div>
    </Transition>
