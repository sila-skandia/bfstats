...
export function generateBattleReport(roundReport: RoundReport): {
  ...
  for (let i = 1; i < snapshots.length; i++) {
      ...
      if (prevPlayer) {
        ...
        if (killsDiff > 0) {
          ...
          if (!firstBloodPlayer) {
            ...
            events.push({
              ...
              message: `FIRST BLOOD! ${decodePlayerName(currentPlayer.playerName)} draws first blood!`,
              color: 'text-red-500',
              icon: '🩸',
              isHighlight: true,
            });
            ...
            highlights.push({
              ...
              description: `${decodePlayerName(currentPlayer.playerName)} draws first blood`,
              icon: '🩸',
            });
          }
          ...
          if (streakInfo && (!prevStreakInfo || streakInfo.name !== prevStreakInfo.name)) {
            events.push({
              ...
              type: 'killing_spree',
              player: currentPlayer.playerName,
              message: `${streakInfo.icon} ${streakInfo.name}! ${decodePlayerName(currentPlayer.playerName)} is on a ${state.currentStreak} kill streak!`,
          ...
          for (let k = 0; k < killsDiff; k++) {
            events.push({
              ...
              type: 'kill',
              player: currentPlayer.playerName,
              message: `${decodePlayerName(currentPlayer.playerName)} eliminated an enemy (+${pointsPerKill} pts)`,
        ...
        if (deathsDiff > 0) {
          ...
          if (state.currentStreak >= STREAK_THRESHOLDS.KILLING_SPREE) {
            events.push({
              ...
              type: 'spree_ended',
              player: currentPlayer.playerName,
              message: `${decodePlayerName(currentPlayer.playerName)}'s ${state.currentStreak} kill streak has ended!`,
              color: 'text-gray-400',
              icon: '💔',
              isHighlight: false,
              value: state.currentStreak,
          ...
          for (let d = 0; d < deathsDiff; d++) {
            events.push({
              ...
              type: 'death',
              player: currentPlayer.playerName,
              message: `${decodePlayerName(currentPlayer.playerName)} was eliminated`,
              color: 'text-red-400',
              icon: '⚰️',
              isHighlight: false,
            });
        ...
        if (scoreDiff > 50 && killsDiff === 0) {
          events.push({
            ...
            type: 'objective',
            player: currentPlayer.playerName,
            message: `${decodePlayerName(currentPlayer.playerName)} completed an objective (+${scoreDiff} pts)`,
            color: 'text-purple-400',
            icon: '🎯',
            isHighlight: false,
            value: scoreDiff,
        ...
        events.push({
          ...
          type: 'spawn',
          player: currentPlayer.playerName,
          message: `${decodePlayerName(currentPlayer.playerName)} joined the battle`,
          color: 'text-blue-400',
          icon: '🪂',
          isHighlight: false,
        });
    ...
    if (i % 5 === 0 && currentSnapshot.entries.length > 0) {
      ...
      events.push({
        ...
        type: 'system',
        player: 'SYSTEM',
        message: `${decodePlayerName(topPlayer.playerName)} leads with ${topPlayer.score} points (${topPlayer.kills}/${topPlayer.deaths})`,
  ...
  if (winner) {
    events.push({
      ...
      type: 'system',
      player: 'SYSTEM',
      message: `Battle concluded! ${decodePlayerName(winner.playerName)} achieved victory with ${winner.score} points!`,
      color: 'text-yellow-400',
      icon: '🏆',
      isHighlight: true,
    });
