    ...
    ILogger<PlayerRelationshipService> logger) : IPlayerRelationshipService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var graph = await neo4jService.ExecuteReadAsync(async tx =>
        {
            ...
            return await BuildTwoHopNetworkAsync(tx, playerName, maxNodes, depth);
        });
    ...
    private async Task<PlayerNetworkGraph> BuildTwoHopNetworkAsync(
        ...
        int depth)
    {
        var allyLimit = Math.Clamp(maxNodes / 6, 8, NetworkGraphAllyLimit);
        var fofPerAlly = NetworkGraphFofPerAlly;
        ...
        if (allyNames.Count > 0)
        {
            ...
            await foreach (var rec in fofCursor)
            {
                ...
                TryAddEdge(
                    edges,
                    edgeKeys,
                    rec["allyName"].As<string>(),
                    fofName,
                    rec["fofWeight"].As<int>(),
                    ToNullableDateTime(rec["fofLastPlayed"]) ?? DateTime.MinValue);
            }
        }

        // Clique among the 8-15 allies only. A cartesian over every discovered
        // name (center + allies + FoF) is O(n^2) Neo4j seeks on the volume.
